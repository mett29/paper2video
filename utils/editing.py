...
def create_beamer(slides_content: List[dict]) -> str:
    ...
    # Generate the PDF
    output_path = os.path.join(config.OUTPUT_DIRPATH, 'beamer_presentation.pdf')
...
def convert_to_video(slides_content: List[dict], presentation_path: str) -> None:
    ...
    This function creates the following intermediate outputs:
    - the audio clips for each slide in MP3 format
    - the video clips for each slide in MP4 format

    Eventually, everything is concatenated and the final video is exported.
    The intermediate files are used by the animate.ipynb notebook.
    """
    with tempfile.TemporaryDirectory() as temp_path:
        ...
        for i, (slide, image) in enumerate(zip(slides_content, images_from_path)):
            ...
            # Save the single video clip
            slide_clip.write_videofile(
                filename=os.path.join(config.VIDEO_CLIPS_PATH, f"frame_{i}.mp4"), fps=24
            )
            clips.append(slide_clip)

        # Concatenate clips and export the final video
        final_clip = concatenate_videoclips(clips)
        final_clip.write_videofile(
            filename=os.path.join(config.OUTPUT_DIRPATH, 'final_video.mp4'), fps=24
        )
